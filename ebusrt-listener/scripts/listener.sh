...
do
	...
	then
		...
		do
			...
			ncftpput -u bkp -p bkp $IPOP_SRT / "$HOME_SRT/LOT/$realfilename"
			mv "$HOME_SRT/LOT/$realfilename" $HOME_SRT/DONE/ -f
			echo " file $HOME_SRT/LOT/$realfilename moved from LOT to DONE folder"
		done
		IFS=$SAVEIFS
